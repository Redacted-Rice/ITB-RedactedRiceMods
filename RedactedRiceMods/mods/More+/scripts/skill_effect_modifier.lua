...
function SkillEffectModifier:modifySpaceDamage(attackingPawn, isFinalEffect, spaceDamage, indexes, targetPawn)
...
function SkillEffectModifier:processDamageList(attackingPawn, isFinalEffect, damageList, indexes, pawnPositions, pendingMoves)
	...
	for i, spaceDamage in ipairs(damageList) do
		...
		-- if its not a movement, apply any pending moves and then call into our modify fn
			...
			local additionalDamages = self:modifySpaceDamage(attackingPawn, isFinalEffect, spaceDamage, indexes, targetPawn)
...
function SkillEffectModifier.processAllSkills(attackingPawn, isFinalEffect, skillEffect, p2)
	...
	-- Build initial effects table from both effect and q_effect
	local effectsTable = extract_table(skillEffect.effect)
	if not skillEffect.q_effect:empty() then
		for _, qDamage in ipairs(extract_table(skillEffect.q_effect)) do
			table.insert(effectsTable, qDamage)
		end
	end
	if #effectsTable == 0 then
		logger.logDebug(SUBMODULE, "No effects to process")
		return
	end

	-- Find all targeted pawns. Not all of these may be affected and some may be pushed
	-- but this is the list of all possible pawns that can be affected directly by
	-- the effect. This does NOT include pawns that are bumped into
	...
	for _, skill in ipairs(registeredSkills) do
		...
		if attackingPilot and cplus_plus_ex:isSkillOnPilot(skill.id, attackingPilot) then
			...
			logger.logDebug(SUBMODULE, "Skill %s applies to attacking pawn %d", skill.id, attackingPawn:GetId())
		end
		...
		for pawnId, targetPawn in pairs(allTargetedPawns) do
			...
			if targetPilot and cplus_plus_ex:isSkillOnPilot(skill.id, targetPilot) then
				...
				if not alreadyAdded then
					...
					logger.logDebug(SUBMODULE, "Skill %s applies to targeted pawn %d", skill.id, targetPawn:GetId())
				end
			end
		end
	end
	...
	if #skillPawnCombos == 0 then
		logger.logDebug(SUBMODULE, "No skills apply to this effect")
		return
	end
	logger.logDebug(SUBMODULE, "Processing %d skill+pawn combinations", #skillPawnCombos)

	-- loop through all skills, then check for new damages and repeat until no new damages are found
	-- or we reach the max passes
	local pawnPositions = {}
	...
	while damagesToProcess and #damagesToProcess > 0 and currentPass < maxPasses do
		...
		for _, combo in ipairs(skillPawnCombos) do
			logger.logDebug(SUBMODULE, "Pass %d: Processing skill %s for pawn %d (%s)",
					currentPass, combo.skill.id, combo.pawn:GetId(), combo.source)

			-- Always pass attackingPawn as first parameter, skill will check which pawn has it
			local newDamages = combo.skill:processDamageList(attackingPawn, isFinalEffect, damagesToProcess,
				combo.indexes, pawnPositions, pendingMoves)
