...
function SkillEffectModifier:modifySpaceDamage(source, attackingPawn, previewState, spaceDamage, indexes, targetPawn)
...
function SkillEffectModifier:processDamageList(source, attackingPawn, isFinalEffect, damageList, indexes, pawnPositions, pendingMoves, isQueued)
	...
	for i, spaceDamage in ipairs(damageList) do
		...
		-- if its not a movement, apply any pending moves and then call into our modify fn
			...
			-- If its a target skill, only call if we have a target with the skill
			...
			if source ~= self.SOURCE_TARGET or (targetPawn and cplus_plus_ex:isSkillOnPawn(self.id, targetPawn)) then
				local additionalDamages = self:modifySpaceDamage(source, attackingPawn, previewState, spaceDamage, indexes, targetPawn)
