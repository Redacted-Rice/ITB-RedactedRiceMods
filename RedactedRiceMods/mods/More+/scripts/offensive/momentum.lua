...
function customSkill:momentumTriggered(pawnId, p1, p2)
	...
	if distance >= MIN_DISTANCE then
		if not pawn:IsBoosted() then
			...
			self.notPreBoosted[pawnId] = true
...
function customSkill.checkMove(mission, pawn, weaponId, p1, p2, skillEffect)
	if weaponId == "Move" then
		...
		if pilot and cplus_plus_ex:isSkillOnPilot(customSkill.id, pilot) then
			...
			local boostDamage = SpaceDamage(p2, 0)
			boostDamage.sScript = [[
				more_plus.SkillActive.skills.RrMomentum:momentumTriggered(]]..pawnId..[[,]]..p1:GetString()..[[,]]..p1:GetString()..[[)
			]]
