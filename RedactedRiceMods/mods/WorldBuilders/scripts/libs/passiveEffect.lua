...
How to Use:
In the function mod:load(options, version) in init.lua after loading your weapons add these two lines:
...
local PASSIVE_EFFECT_BASE_FN_NAME = "GetPassiveSkillEffect" 
...
function passiveEffect:addPassiveEffect(weapon, hook, weaponIsNotPassiveOnly)
	...
	if not weaponIsNotPassiveOnly then
		...
		if addPassiveEffectDebug then LOG("Auto passive weapon detected") end
	end
		
	--if they pass a table, add it for each hook
	if type(hook) == "table" then
		if addPassiveEffectDebug then LOG("addPassiveEffect table of hooks for "..weapon) end
		for _,singleHook in pairs(hook) do
			self:addPassiveEffect(weapon, singleHook, weaponIsNotPassiveOnly)
		end
	else
		if addPassiveEffectDebug then LOG("addPassiveEffect hook" .. hook .. " for "..weapon) end
		...
		if addPassiveEffectDebug then LOG("Finished adding " .. hook .. " for "..weapon) end
